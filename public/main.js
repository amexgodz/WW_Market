...
function createSkinCard(skin, isInProfile, indexInInventory) {
...
function renderAuthModal() {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            state.isAuthModalOpen = false;
            render();
            showToast(state.authMode === 'login' ? 'Вы успешно вошли' : 'Аккаунт создан, добро пожаловать', 'success');
        }
        catch (error) {
            const err = error;
...
async function bootstrap() {
    window.addEventListener('popstate', (event) => {
        const nextView = event.state?.view ?? 'market';
        state.currentView = nextView;
        render();
    });
    render();
    try {
        const skins = await apiFetch('/api/skins');
        state.skins = skins;
    }
    catch {
        state.skins = [];
